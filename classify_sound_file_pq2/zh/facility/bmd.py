...
def parseMsgs(cacheFolder):
    ...
    for msgF in msgFile:
        shouldJoinLine = True
        if(msgF == 'msg_combine.bmd.msg'):
            shouldJoinLine = False
        msgData = parseMsgFile(os.path.join(cacheFolder, msgF), joinMsgOneLine=shouldJoinLine)
        msgMap[msgF] = msgData
    return msgMap
...
reBuildBinRoot = Path().joinpath(rebuildCPKRoot, "facility")
